        ...
        if ($conn->query($sqlinsertpayment) && $conn->query($sqpupdatecart)) {
            ...
            if ($result->num_rows > 0) {
                while ($rows = $result->fetch_assoc()) {
                    $sbjid = $rows['subject_id'];
                    $sbjqty = $rows['cart_qty'];
                    $updatesubjectsqty = "UPDATE tbl_subjects set subject_qty = (subject_qty-$sbjqty) WHERE subject_id = '$sbjid'";
                    $conn->query($updatesubjectsqty);
                }
            }
